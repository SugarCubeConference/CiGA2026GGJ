...
namespace MaskGame.Managers
{
    ...
    public class GameManager : MonoBehaviour
    {
        ...
        private void InitShadow()
        {
            ...
            shadowIds = new Dictionary<EncounterData, int>(encounterCount);
            shadowDefs = new Kernel.EncounterDefinition[encounterCount];
        ...
        private void CheckShadow(string context)
        {
            ...
            if (!mismatch && expectedPhase == Kernel.GamePhase.WaitAns)
            {
                if (currentEncounter == null
                    || !shadowIds.TryGetValue(currentEncounter, out int encounterId)
                    || encounterId != shadowState.EncId)
                {
                    mismatch = true;
                }
            }
